...
/**
 * Animates the movement of a walker on a grid. The grid is shown with the
...
public class GridMap extends JPanel {
	...
	public GridMap(RandomWalk walk, int gridSize) {
		...
		setPreferredSize(new Dimension(displaySize + offset, displaySize
				+ offset));
		startAnimation();
	}
	...
	public void paintComponent(Graphics obj) {
		...
		page.fillRect(where.x * blockSize + offset - blockSize / 8, where.y
				* blockSize + offset - blockSize / 8, blockSize / 4,
				blockSize / 4);
	}
	...
	private void drawPath(Graphics2D page) {
		...
		for (Point next : path) {
			page.drawLine(prev.x * blockSize + offset, prev.y * blockSize
					+ offset, next.x * blockSize + offset, next.y * blockSize
					+ offset);
			prev = next;
		}
